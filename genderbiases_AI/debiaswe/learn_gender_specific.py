...
gender_seed = set(w for i, w in enumerate(E.words) if w in gender_seed)
...
C = 1.0
clf = LinearSVC(C=C, tol=0.0001,dual='auto', class_weight='balanced')
# Debugging: Check balance in training labels
print("Class distribution in training data:", np.unique(y, return_counts=True))

# Adjustments might be needed based on the output
...
# Save learned gender-specific words to JSON file with error handling
try:
